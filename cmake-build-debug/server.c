        ...
        switch(command){ // Si comporta in base al case
            ...
            case 'o': // Apro il file
                ...
                if(fileOpen == NULL && flags == 0){ // Il file non esiste
                    ...
                    newFile->name[strlen(basename(operations))] = '\0';
                    newFile->size = 0;
                    newFile->buff = NULL;
            ...
            case 'l': // case lock
                ...
                SYSCALL_EXIT("safeWrite", use, safeWrite(fileDescriptorConnection, &resultLock, sizeof(int)), "write", "");
                break;
            case 'u': // case unlock
...
int main(int argc, char* argv[]){
    ...
    int errorPrint;
    errorPrint = printLog(fileLog, logMutex, "Il server ha appena finito di settare la gestione sigaction");
    if(errorPrint == -1){
        fprintf(stdout, "Errore nella print");
    }
    // Le inizializzo a 0
    ...
    while(!isSingint){
        ...
        for(i = 0; i <= fdMax; i++){ // Cerchimo di capire da chi abbiamo ricevuto la richiesta
            if(FD_ISSET(i, &tmpSet)){
                long fileDescriptorConnection;
                if(i == fileDescriptorSocket){ // E' una nuova richiesta di connessione dal socket
                    ec_meno1((fileDescriptorConnection = accept(fileDescriptorSocket, (struct sockaddr*)NULL, NULL)), "accept"); // Accetto la richiesta
                    FD_SET(fileDescriptorConnection, &fileDescriptorSet); // Aggiungo al master set
                    numberActive++;
                ...
                tmpCommand->operations[strlen(operationStr)] = '\0';
                free(operationStr);
                pthreadMutexLock(&mutexClientsList);
                ec_meno1((addTail(&clientsList, tmpCommand)), "addTail");
                pthreadMutexUnlock(&mutexClientsList);
