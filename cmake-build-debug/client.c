...
int openConnection(const char* sockname, int msec, const struct timespec abstime){
    ...
    serverAddress.sun_path[strlen(socketNameConfig)] = '\0';
...
int appendToFile(const char* pathname, void* buff, size_t size){
    ...
    if(!check){
        fprintf(stderr, "Non ho trovato alcun file con questo nome %s\n", pathname);
        return -1;
    }
...
int execCommand(Command *command){
    ...
    if(command->set == 'c'){
        ...
        if(activateStandard){
            fprintf(stdout, "Sto per leggere %d file\n", command->n);
        }
        if(readNFiles(command->n, savePath) == -1){
            return -1;
        }
    } else if(command->set == 'W'){
        if(activateStandard){
            fprintf(stdout, "Scrivo il file %s sul server\n", command->name);
        }
    ...
    while (L->size > 0){
        ...
        if(command->set == 'W'){
            ...
            if(execCommand(command) == -1){
                fprintf(stderr, "Errore di esecuzione del comando %c\n", command->set);
            }
            free(command->name);
        }
        free(command);
